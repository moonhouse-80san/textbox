    ...
    class textbox extends EditorHandler {
        ...
        public function transHTML($xml_obj): string {
            ...
            if($use_folder == "Y") {
                ...
                $output .= sprintf('<div style="%s" id="folder_%s" data-source="%s"><pre class="%s">%s</pre><button onclick="copyTextboxContent(\'folder_%s\')" style="%s">복사하기</button></div>', 
                    $style, 
                    $folder_id, 
                ...
                $output .= sprintf('<div style="%s position: relative;" id="%s" data-source="%s"><pre class="%s">%s</pre><button onclick="copyTextboxContent(\'%s\')" style="%s">복사하기</button></div>', 
                    $style, 
                    $box_id, 
                    htmlspecialchars($body, ENT_QUOTES), 
                    $content_class,
